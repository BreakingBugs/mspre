...
struct paquete identificadorPaquetesCliente(char * charPaquete) {
    ...
    if (cant == 3) {
        ...
        //TODO: Asignar paquete de error
        printf("Error en el paquete recibido");
    }
    return packet;
}
...
struct paquete identificadorPaquetesServidor(char *charPaquete) {
    ...
    /*como en esta primera llamada a strtok estrajimos un dato 
     *ponemos nuestra variable cant a 1 para poder diferecnciar
     * el paquete de acuerdo a la cantidad de datos que contenga 
     */
    cant = contador(charPaquete);
...
char* prepararPaquete(struct paquete *packet, char *buf) {
 ...
 char* agregarSeleccion(struct paquete *packet,char *buf){
     char var[6]; 
     sprintf(var, "%d", packet->seleccion);
        strcat(buf, "::");
        strcat(buf, var);
 ...
 char* divisorDeEstructuras(char* aux){
	char ayu[2]="::", aux2[1024];
	char* resp = (char *)malloc(sizeof(char)*1024);
	int len,j = 0,k,l=0,i,p=0;
	len = strlen(aux);
	for(i =0 ; i <= len; i++){
		if(aux[i] == ayu [j] && aux[i+1] == ayu[j+1]){
			for(k=i+2 ; k<= len; k++){
				 aux2[l] = aux[k];
				 l++;
			}
